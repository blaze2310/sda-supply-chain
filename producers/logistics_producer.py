...
logistics_data = pd.read_excel(FILE_PATH).head(SAMPLE_SIZE)

print(
    f"Streaming {len(logistics_data)} logistics records "
    f"to {TOPIC}...\n"
)
...
for index, row in logistics_data.iterrows():
    ...
    entity_id = (
        ...
        or f"SHIP-{index + 1:06d}"
    )

    event = {
        "event_id": f"SHIP-EVENT-{index + 1:06d}",
